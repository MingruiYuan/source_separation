...
import time
import json
from utils import compute_validation
OP_POINT = 65280
imgnum = 14.2106
test_dir = '../testset25/'
outdict = {}
...
def init(ROOT_DIR = '../module/', DICT = '500.t7', CUDA=1):
   #ROOT_DIR = './'
   #ROOT_DIR = 'train_final_31-21-18-41_train_log/'
   #DICT = '300.t7'
...
def image_segmentation(img):
	lap = cv2.Laplacian(img[:,:,0], cv2.CV_64F)
	line = np.argmax(np.sum(np.abs(lap),0))
	if line < 5 or line > np.shape(lap)[1]-5:
		line = np.argmax(np.sum(np.abs(lap[5:np.shape(lap)[1]-5]),0))+5

	return img[:,:line,:], img[:,line:,:]

def SPLtest(image, audio, output_dir, files, val=False, gt_dir=None):
	if val:
		fs1, audio_gt1 = wavfile.read(gt_dir+files+'_gt1.wav')
		fs2, audio_gt2 = wavfile.read(gt_dir+files+'_gt2.wav')
		audio_gt1 = sn.zoom(audio_gt1.astype('float64'), 0.25, order=2)
		audio_gt2 = sn.zoom(audio_gt2.astype('float64'), 0.25, order=2)
	...
	for p in range(curr_N+1):
		...
		if p == curr_N:
			...
			if x1 < np.shape(image)[0]:
				im_L1, im_R1 = image_segmentation(image[x1])
			...
			if x2 < np.shape(image)[0]:
				im_L2, im_R2 = image_segmentation(image[x2])
			...
			if x3 < np.shape(image)[0]:
				im_L3, im_R3 = image_segmentation(image[x3])
			...
			im_L2, im_R2 = image_segmentation(image[x2])
			# cv2.imwrite(save_dir+str(cnt)+'_'+str(x2)+'_left.jpg',im_L2)
	...
	img_out_right = imageNet(image_right[:,0,:,:,:], image_right[:,1,:,:,:], image_right[:,2,:,:,:])
	...
	print('SDR:', (sdr_left[0]+sdr_right[0])/2)
	...
	d1 = {"position":0,"audio":files+'_seg1.wav'}
	d2 = {"position":1,"audio":files+'_seg2.wav'}
	outdict[files+'.mp4']=[d1,d2]

	if ACCURACY:
		sdr_left_right = compute_validation(audio_gt2, audio_proc_left)
		sdr_right_left = compute_validation(audio_gt1, audio_proc_right)
		if sdr_left > sdr_left_right and sdr_right > sdr_right_left:
			acc = True
		else:
			acc = False
		return acc, (sdr_left[0]+sdr_right[0])/2
	else:
		return False, (sdr_left[0]+sdr_right[0])/2

#if __name__ == 'main':
start2 = time.time()
filename = os.listdir(test_dir+'testimage/')
output_dir = '../separated_audio/test_25/'
...
for files in filename:
	...
	accu, sdru = SPLtest(image, audio, output_dir, files, True, gt_dir)
	total_avg_sdr += sdru
	acc += accu
	cnt += 1
end2 = time.time()
print('time elapsed:', end2-start2)
#json.dumps(outdict,sort_keys=True,indent=4,separators=(',',':'))
with open('../json/sep_25.json','w') as f:
	json.dump(outdict, f, sort_keys=False, indent=4, separators=(',', ':'))
print('total_avg_sdr:', total_avg_sdr / 25)
if ACCURACY:
	print('Accuracy:', acc/25)
